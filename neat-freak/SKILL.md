...
> **定位（2026-07-08 起）：backstop，不是主机制。** 知识同步的主机制是 in-flight——让文档过期的那次编辑应当在**同一批工作里**顺手更新文档（见 `rules/workflow.md` → Knowledge sync）。本 skill 是收尾兜底：理想情况下扫描结果应该是**零发现**。如果扫出了非琐碎的漂移，说明本次任务的 in-flight 纪律失守——修完后顺带记一句什么漏了、为什么漏。
...
### 第五步补充：下一会话起手卡（仅在 handoff 语义触发时输出）

**何时输出**：用户的触发词带 "handoff" / "交接" / "让接班 agent 接手" / "下一个会话能直接上手" / 类似含义时。普通的 `/sync` / `整理一下` / `更新记忆` **不需要**这一段——它是给"下一会话"看的便携小抄，不是日常同步的产物。

**输出形式**：**纯聊天文本，不写文件**。绝不创建 `HANDOFF.md` / `STATUS.md` / `NEXT-SESSION.md` 之类——这类文件会被 autonomous-grind hooks 直接拦截，而且和 auto-memory 职责重叠。起手卡是"用户复制到新会话粘贴一次就丢"的瞬时上下文。
...
**当前状态**：1 句话——分支 / PR / CI / 阻塞点等具体定位
**下一步**：1-3 条，按优先级排
- [ ] xxx
- [ ] xxx
**容易踩的非显而易见点**：0-3 条，**仅写新会话默认想不到的**（已写进 docs 的不要重复）
- xxx
**关键 reference**：1-3 条 file:line 或 commit SHA
- `<path:line>` — 为什么重要
```

**写作要点**：
- 内容是 **auto-memory 不会自动覆盖的瞬时工作态**（"现在卡在哪""下一步是什么"），不是稳定事实（那些 auto-memory 自己会拿）
- 长度上限：120 字以内，能一眼扫完
- 不重复 docs / CLAUDE.md / 记忆里已经写过的内容——那些下个 Agent 自己会读
