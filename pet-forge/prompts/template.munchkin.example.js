...
//
export const CHARACTER_PREFIX = `A cute chibi/kawaii style golden tabby British Shorthair Munchkin cat in 3/4 sitting view.

Coat: cream-gold body with classic mackerel tabby stripes (thin darker-gold vertical stripes on flanks, M-marking on forehead). White chin, bib, and belly.

Body: chunky and round (chubby loaf shape), with EXTRA-SHORT stubby legs deliberately exaggerated — legs visibly too short for the body. This is the signature Munchkin trait and must be kept in every frame.

Tail: short and round-tipped with faint stripe rings.

Head: large and round (≈55% of body height — chibi proportions). Small triangle ears with pink inner.

Eyes: large oval JADE DARK GREEN with one white sparkle highlight. NOT yellow, NOT amber, NOT gold.
...
Art style: clean kawaii vector cartoon with thick (3-4px) dark warm-brown outlines, flat color fills. NO pixel art, NO realistic rendering, NO 3D, NO airbrush.

Background: plain solid green (#00B140) chroma-key.

Consistency contract: every frame must show the SAME chubby short-legged jade-green-eyed gold tabby — same proportions, same eye color, same stripe pattern, no cheek blush.`;
...
export const ANIMATIONS = {
  ...
  'idle-dozing': {
    ...
    duration: 5,
    refKey: 'main',
    prompt: `The character is in its standard sitting pose, looking DROWSY AND SLEEPY. The cat is fighting off sleep — eyes slowly droop down to half-closed (heavy sleepy lids), pause briefly almost shut, then slowly reopen part-way, then droop again, in a slow drowsy rhythm. Head occasionally nods softly downward as if dozing off, then jerks back up gently. Body has very gentle slow breathing. Tail tip flicks lazily once or twice. Calm drowsy mood, NOT alert and NOT energetic. Seamless loop animation, the last frame connects perfectly back to the first frame. Body stays in place — the only motion is the eyes drooping/reopening, gentle head nod, slow breathing, and lazy tail flick. The chubby Munchkin body adds to the cozy drowsy atmosphere.`,
    notes: '困倦待机: 眼皮重半闭+轻点头+慢呼吸, 体现 dozing 而非 alert',
  },
  ...
  'working-typing': {
    ...
    duration: 3,
    refKey: 'main',
    prompt: `The character is rapidly tapping its hands/paws up and down as if typing on an invisible keyboard. The hands alternate left-right in a fast rhythmic pattern. Focused, slightly intense expression. Tail sways gently. Seamless loop animation — the last frame connects perfectly back to the first frame. Body stays in place, only the typing parts move. The short legs make the typing pose visibly comedic — paws barely lift off the surface, body close to ground.`,
  ...
  'notification': {
    ...
    duration: 2.5,
    refKey: 'main',
    prompt: `The character is in its standard sitting pose. SIMPLE MOTION ONLY: ears perk up alert and rotate slightly forward, head tilts up just slightly, eyes widen briefly with surprised expression. Holds the alert pose for a moment. Then ears and head smoothly return to normal pose with eyes back to standard. 2.5 seconds total. CRITICAL: smooth clean motion with NO ghosting, NO double-imaged transitional frames, NO body crouch, NO morphing of body shape. Body stays completely in place — only ears, head tilt, and eye expression change. NO exclamation mark drawn in the video (will be added in post-processing if needed). Starting pose and ending pose match EXACTLY.`,
    notes: '极简警觉: 只动耳朵+微抬头+睁大眼, 避免重影; 身体完全不动',
  },
  ...
  'error': {
    ...
    duration: 5,
    refKey: 'error',
    prompt: `CRITICAL POSE: the character is LYING DOWN ON ITS SIDE OR BACK, body fully horizontal on the ground (NOT sitting up, NOT in a sleep curl — flat-on-the-ground knockout). Eyes are drawn as cartoon X-shapes (× ×) — two crossed thick black lines per eye, the classic 'knocked out' convention. Mouth slightly open with tongue tip lolling out one side comically. Above the head, small puff cloud and 2-3 sparkle stars drift slowly. Body has very gentle slow breathing — slow subtle rise and fall while lying down. Stays in same horizontal lying position the entire time. Seamless loop, calm and slow. DO NOT change to sitting pose, DO NOT animate to a different pose — the cat REMAINS LYING DOWN. Only the breathing, sparkle stars, and puff cloud animate.`,
    notes: '强调侧躺不起来+XX眼+晕眩星星, 循环',
  },
